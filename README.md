...
```
2 次尝试，2 次都回 'Please try again.'
```

而且这次会话里格状题极其稀少：**67 轮里只出 2 道**（约 3%），
所以样本量根本不足以判断判据改进是否真的有效。

也正因如此，`tools/eval_grid.py` 的两次 10 题统计（**均为 0/10**）
在当前题型分布下都难以复现——上一次能凑满 10 题，这一次 67 轮只有 2 题。

**结论：判据的改进在离线自检上成立（n=1），但在 live 上没有任何证据支持它更有效（n=2，全错）。**
我不把它算作进展，只能说"有一个机制上更合理、且在唯一可核验样例上正确的候选判据"。
