...
过度分割（一个角色被拆成多个连通域），取到的块可能只是角色的一部分（比如一条胳膊），
这会直接污染相似度。

所以结论是：**这是一个新发现的结构上更有潜力的题型，但还没有解出**。
下一步该做的是：先修好格提取（把同一格内的多个块按格子边界合并，
或直接用白色方块底检测格线），再重做相似度——那时信号强度才被真正测到。

工具：`research/grid_characters.py`。实例图：`research_out/instances/pat_0[1-8].png`。
